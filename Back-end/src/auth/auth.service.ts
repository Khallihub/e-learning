...
export class AuthService {
  ...
  async loginUser(dto: loginDto): Promise<any> {
    const query = { username: dto.email, pass: dto.password };
    ...
    if (!user) {
      ...
      const tokens = await this.getTokens(user.email, user.role);
      await this.updateRtHash(user.id, tokens.refresh_token);
      return [{ ...tokens, role: user.role },user.enrolledCourses, user.role];
    }
  }
  ...
  async refreshTokens(ref: { email: string; rt: string }) {
    const query = { username: ref.email, pass: 'null' };
    console.log(query.username, 'hdjks');
    const user = await this.usersService.findUser(query);
    // console.log(user)
